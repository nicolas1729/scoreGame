...
import { Trophy, RotateCcw, Plus, Minus, Edit2, Save, History, Trash } from 'lucide-react';
...
const STORAGE_KEY = 'flip7-score-tracker';

export default function App() {
  const [players, setPlayers] = useState<Player[]>([
    { id: 0, name: 'Joueur 1', score: 0, history: [] },
    { id: 1, name: 'Joueur 2', score: 0, history: [] },
    { id: 2, name: 'Joueur 3', score: 0, history: [] },
  ]);
  ...
  const handleAddCustom = (id: number) => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-cyan-500/30 p-4 md:p-8">
      <header className="max-w-5xl mx-auto mb-12 flex justify-between items-end border-b border-white/10 pb-6">
        <div>
        ...
        <button 
          onClick={resetScores}
          className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-red-500/20 hover:text-red-400 border border-white/10 rounded-lg transition-all text-sm font-medium"
        >
      ...
      <main className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        {players.map((player) => (
          <motion.div 
            key={player.id}
            layout
            className="bg-[#141414] border border-white/10 rounded-2xl overflow-hidden flex flex-col"
          >
            {/* Player Header */}
      ...
      <footer className="max-w-5xl mx-auto mt-12 pt-8 border-t border-white/5 flex justify-between items-center text-white/20 text-[10px] font-mono uppercase tracking-[0.2em]">
        <div>Flip 7 Score Manager v1.0</div>
        <div>Sauvegarde Automatique Active</div>
      </footer>
    </div>
  );
}
